...
// ── Simple in-memory rate limiter for POST /login ──────────────────────────
// Max 10 attempts per IP per 5 minutes
const LOGIN_RATE_LIMIT = new Map(); // ip -> { count, resetAt }
...
function checkLoginRateLimit(ip) {
  ...
  entry.count += 1;
  if (entry.count > LOGIN_MAX_ATTEMPTS) return false; // blocked
  return true;
}
...
function isTurnstileEnabled(req) {
  // ✅ Dev/localhost: ปิด Turnstile ทั้ง backend และ frontend
  if (isLocalRequest(req)) return false;

  // ✅ Production: เปิดเมื่อมีทั้ง site key และ secret key
...
async function verifyTurnstile(token, remoteIp) {
  ...
  try {
    ...
    glog.error('Turnstile verify error', err);
...
router.get('/login', (req, res) => {
  ...
  const turnstileEnabled = isTurnstileEnabled(req);

  res.render('auth/login', {
    title: 'เข้าสู่ระบบ',
    next,
    turnstileEnabled,
    turnstileSiteKey: turnstileEnabled ? (config.turnstile?.siteKey || '') : '',
  });
});
...
router.post('/login', parseUrlencoded, async (req, res) => {
  try {
    ...
    if (!checkLoginRateLimit(clientIp)) {
      return res.status(429).json({ ok: false, message: '⚠️ พยายามเข้าสู่ระบบมากเกินไป กรุณารอ 5 นาทีแล้วลองใหม่' });
    }
    ...
    // ✅ Production เท่านั้น: localhost/dev ไม่ต้องใช้ Turnstile
    if (isTurnstileEnabled(req)) {
    ...
    // ✅ จัดเก็บ Session พร้อม regenerate เพื่อป้องกัน session fixation
    const sessionUser = {
      _id: String(user._id),
