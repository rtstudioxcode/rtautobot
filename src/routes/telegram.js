...
function classifyTelegramWorkLogText(text) {
  const s = String(text || "").trim();
  if (!s) return null;

  // ไม่นับ log ระบบ/หัวท้ายงาน เพราะไม่ใช่ target/member attempt
  if (/^(เริ่มงาน|กำลังโหลด|คืนเครดิต)/i.test(s)) return null;
  if (/จบงาน|ดึงสมาชิกสำเร็จ|หยุดงาน|งานนี้ถูกยกเลิก|บัญชีทั้งหมดติด|รายชื่อในรายการถูกใช้งานจนครบ/i.test(s)) return null;

  if (/✅/.test(s) && /เชิญ/i.test(s) && /สำเร็จ/i.test(s) && !/ไม่สำเร็จ/i.test(s)) return "success";
  if (/❌/.test(s) && /(เชิญ|ข้าม|หา user|resolve|import|ไม่สำเร็จ)/i.test(s)) return "failed";
  if (/^(ข้าม|skip)/i.test(s) && /(หา user|resolve|import|ไม่ได้|ไม่เจอ)/i.test(s)) return "failed";
  if (/เชิญ/i.test(s) && /ไม่สำเร็จ/i.test(s)) return "failed";

  return null;
}

function calcTelegramWorkStats(logs = []) {
  let success = 0;
  let failed = 0;
  for (const line of (Array.isArray(logs) ? logs : [])) {
    const type = classifyTelegramWorkLogText(line?.text || line);
...
function markJobFailedAttempt(job, n = 1) {
  if (!job) return;
  job.failed = Number(job.failed || 0) + Number(n || 1);
}
...
async function safeConnect(client) {
  for (let i = 0; i < 3; i++) {
    try {
      ...
      glog.log("[safeConnect] fail:", e.message);
      await sleep(1500);
    }
  }
  return false;
...
async function safeClose(client) {
  ...
  try {
    ...
    glog.log("destroy fail:", e.message);
  }
}
...
export async function startTelegramJob(jobId) {
  ...
  try {
    ...
    if (stopRequested.has(String(jobId)) || String(job.status || "").toLowerCase() === "stopped") {
      telegramPush(jobId, { status: "stopped", log: "⛔ งานนี้ถูกยกเลิกแล้ว" });
      return;
    }
        ...
        try {
          ...
          // --- MODE: LIST ---
            ...
            for (; listIndex < targets.length; listIndex++) {
              ...
              if (!rr.ok) {
                const label = t?.value || t?.input || t?.raw || t?.phone || t?.username || 'unknown';
                const reason = rr.reason || 'resolve failed';
                pushJobLog(job, jobId, `❌ ข้าม ${label}: หา user ไม่เจอ/resolve ไม่ได้ — ข้อผิดพลาด: ${reason}`, { persist: true });
                markJobFailedAttempt(job);
                continue;
              }
...
export async function stopTelegramJob(jobId, by = "user") {
  ...
  // กันซ้ำ: หยุดได้เฉพาะงานที่ยังทำงานอยู่เท่านั้น
  const currentStatus = String(job.status || "").toLowerCase();
  if (["stopped", "finished", "auto_stopped", "failed", "error"].includes(currentStatus)) {
...
function log(...args) {
  if (GLOBAL_LOG) glog.log('[STREAM]', ...args);
}
...
router.get("/accounts", requireAuth, async (req, res) => {
  try {
    ...
    glog.error("TG ACCOUNT LIST ERROR:", err);
...
router.post("/accounts/login/start", requireAuth, async (req, res) => {
  try {
    ...
    glog.error("START_LOGIN_ERROR:", err);
  ...
  try {
    ...
    if (!acc) {
      acc = new TgAccount({
        ...
        apiId: data.apiId,
        apiHash: data.apiHash,
        session: result.sessionString, // TODO: encrypt session before storing
        status: "READY",
        invitesToday: 0,
      ...
      acc.session = result.sessionString; // TODO: encrypt session before storing
      acc.status  = "READY";
    ...
    glog.error("FINISH LOGIN ERROR:", err);
    return res.json({ ok:false, error:"เกิดข้อผิดพลาด" });
  }
});
...
router.post("/jobs/start", requireAuth, async (req, res) => {
  try {
    ...
    glog.log("[/telegram/jobs/start] body =", req.body);
    /* =========================
      VALIDATE INPUT
    ========================== */
    ...
    glog.error("START_JOB_ERROR:", err);
    return res.json({ ok:false, error:"เกิดข้อผิดพลาด" });
  }
});
...
router.post("/jobs/:jobId/stop", requireAuth, async (req, res) => {
  try {
    ...
    const job = await TelegramJob.findById(jobId).select("userId status").lean();
    ...
    const result = await stopTelegramJob(jobId, "user");
    return res.status(result.ok ? 200 : 400).json(result);

  } catch (err) {
    glog.error("STOP_JOB_ERROR:", err);
    return res.json({ ok:false, error:"เกิดข้อผิดพลาด" });
  }
});
...
router.get("/jobs/:jobId/stream", requireAuth, async (req, res) => {
  ...
  // 🔥 กันเปิดหลาย connection ต่อ user
  const key = `tg_stream_${uid}`;

  if (global[key]) {
    try {
      global[key].end();
      log('KILL OLD STREAM', uid);
    } catch {}
  }
...
router.get("/history", requireAuth, async (req, res) => {
  try {
    ...
    // ✅ โหลดเฉพาะข้อมูลที่ใช้แสดงการ์ดก่อน ไม่ดึง logs ทั้งก้อน
    // logs จะโหลดเฉพาะตอนกดปุ่มรายละเอียดผ่าน /telegram/history/:jobId
    const history = await TelegramJob.find({ userId: uid })
      .select("orderId srcGroup destGroup invited limit failed status createdAt updatedAt")
      .sort({ createdAt: -1 })
      .limit(50)
      .lean();
    ...
    glog.error("Error fetching history:", err);
...
/* ===============================================================
   API: HISTORY LIST
   ✅ ลด Traffic:
   - ไม่ส่ง logs ใน list แล้ว เพราะ logs ใหญ่มาก
   - จำกัดจำนวนรายการต่อหน้า
   - ใส่ Cache-Control + ETag ให้ browser ใช้ cache ได้
=============================================================== */
router.get("/history/list", requireAuth, async (req, res) => {
  try {
    ...
    const latest = await TelegramJob.findOne({ userId: uid })
      .select("updatedAt createdAt status invited limit")
      .sort({ updatedAt: -1, createdAt: -1 })
      .lean();

    const latestKey = latest
      ? [latest.updatedAt || latest.createdAt, latest.status, latest.invited, latest.limit].join(":")
      : "empty";

    const etag = `W/"tg-history-list-${uid}-${page}-${limit}-${Buffer.from(String(latestKey)).toString("base64url")}"`;
    ...
    glog.error("TG HISTORY LIST ERROR:", err);
    return res.status(500).json({ ok: false, error: "เกิดข้อผิดพลาด" });
  }
});


/* ===============================================================
   API: STOP TELEGRAM JOB FROM HISTORY
   หยุดเฉพาะงาน jobId ที่กดเท่านั้น ไม่กระทบงานอื่นของผู้ใช้คนอื่น
=============================================================== */
router.post("/history/:jobId/stop", requireAuth, async (req, res) => {
  try {
    const uid = req.session.user._id;
    const jobId = req.params.jobId;

    const job = await TelegramJob.findById(jobId).select("userId status").lean();
    if (!job) return res.status(404).json({ ok:false, error:"ไม่พบงาน" });

    if (String(job.userId) !== String(uid)) {
      return res.status(403).json({ ok:false, error:"ไม่มีสิทธิ์หยุดงานนี้" });
    }
    ...
    glog.error("TG_HISTORY_STOP_ERROR:", err);
    return res.status(500).json({ ok:false, error:"เกิดข้อผิดพลาด" });
  }
});

router.post("/history/:jobId/cancel", requireAuth, async (req, res) => {
  req.url = `/history/${req.params.jobId}/stop`;
...
router.get("/history/:jobId", requireAuth, async (req, res) => {
  ...
  const stats = calcTelegramWorkStats(logs);
...
router.post("/targets/parse", requireAuth, upload.single("file"), (req, res) => {
  try {
    ...
    glog.error("parse targets error:", e);
