  ...
  try {
    ...
    if (!cats.length) {
      try {
        ...
        glog.log(`✅ Auto-sync services on first visit: ${r.count} items`);
        cats = await Category.find().sort({ name: 1 }).lean();
      } catch (e) {
        glog.error('❌ Auto-sync failed:', e?.response?.data || e.message || e);
...
router.get('/c/:cat', async (req, res) => {
  try {
    ...
    glog.error('GET /c/:cat error:', err);
    return res.status(500).send('internal error');
  }
});
...
router.get('/c/:cat/:sub', async (req, res) => {
  try {
    ...
    await Promise.all(
      services.map(async (s) => {
        try {
          ...
          glog.warn(`computePrice failed for service ${s._id}:`, e?.message || e);
    ...
    glog.error('GET /c/:cat/:sub error:', err);
    return res.status(500).send('internal error');
  }
});
...
router.get('/service/:id', async (req, res) => {
  try {
    ...
    try {
      ...
      glog.warn(`computePrice failed for service ${svc._id}:`, e?.message || e);
    ...
    glog.error('GET /service/:id error:', err);
    return res.status(500).send('internal error');
  }
});
