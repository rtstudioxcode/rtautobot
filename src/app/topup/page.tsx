...
export default function TopupPage() {
      ...
      try {
        ...
        if (j?.ok && ['completed', 'failed', 'cancelled', 'canceled', 'reject'].includes(j.status)) {
          ...
          if (j.status === 'completed') {
            ...
            notifyMsg(setMsg, { type: 'ok', text: `เติมเงินสำเร็จ!${amt != null ? ` เพิ่มเครดิต ฿${Number(amt).toLocaleString(undefined, { minimumFractionDigits: 2 })}` : ''}` });
            setQrModal(null); setQrExpired(false); loadPage();
          } else {
            notifyMsg(setMsg, { type: 'err', text: j.status === 'failed' ? 'การชำระล้มเหลว กรุณาลองใหม่' : 'รายการถูกยกเลิก' });
            setQrModal(null);
          }
          return;
        }
  ...
  async function submitTopup(e, walletCode) {
    ...
    if (!base || base < 1) { notifyMsg(setMsg, { type: 'err', text: 'จำนวนเงินขั้นต่ำ 1 บาท' }); return; }
    ...
    try {
      ...
      if (!res.ok || !j?.ok) { notifyMsg(setMsg, { type: 'err', text: j?.message || 'สร้างคำสั่งเติมเงินไม่สำเร็จ' }); setBusy(false); return; }
    ...
    } catch (err: any) { notifyMsg(setMsg, { type: 'err', text: `ขัดข้อง: ${err.message}` }); }
    setBusy(false);
  }
  ...
  async function cancelTopup(txId) {
    ...
    try {
      ...
      notifyMsg(setMsg, { type: 'ok', text: 'ยกเลิกรายการแล้ว สามารถสร้างรายการใหม่ได้เลย' });
      stopTxWatcher(); clearCountdown(); setQrModal(null); setQrExpired(false); loadPage();
    } catch (err: any) { notifyMsg(setMsg, { type: 'err', text: err.message }); }
  }
