...
export default function AccountPage() {
  ...
  async function saveProfile(e) {
    ...
    const data = await res.json();
    setBusy(false);
    if (!data.ok) { notifyMsg(setMsg, { type: 'error', text: data.error || data.message || 'บันทึกไม่สำเร็จ' }); return; }
  ...
  async function changePassword(e) {
    ...
    if (form.get('newPassword') !== form.get('confirmPassword')) {
      notifyMsg(setMsg, { type: 'error', text: 'รหัสผ่านใหม่ไม่ตรงกัน' }); setBusy(false); return;
    }
    ...
    const data = await res.json();
    setBusy(false);
    notifyMsg(setMsg, data.ok ? { type: 'success', text: 'เปลี่ยนรหัสผ่านสำเร็จ' } : { type: 'error', text: data.error || data.message || 'ไม่สำเร็จ' });
  ...
  async function uploadAvatar(e) {
    ...
    const data = await res.json();
    setBusy(false);
    if (!data.ok) { notifyMsg(setMsg, { type: 'error', text: data.message || 'อัปโหลดไม่สำเร็จ' }); return; }
    notifyMsg(setMsg, { type: 'success', text: 'เปลี่ยนรูปสำเร็จ' });
  ...
  async function createAffiliateLink() {
    ...
    setBusy(false);
    if (data.ok) {
      notifyMsg(setMsg, { type: 'success', text: 'สร้างลิงก์แนะนำเพื่อนสำเร็จ' });
      setUser((u) => ({ ...u, affiliateKey: data.key || u.affiliateKey }));
      loadAffiliate();
    } else {
      notifyMsg(setMsg, { type: 'error', text: data.error || 'สร้างลิงก์ไม่สำเร็จ' });
    }
  }

  async function redeemPointsToBalance() {
    if (busy) return;
    const ok = await confirmAction({
      title: 'แลกแต้มเป็นเครดิต',
      message: 'ยืนยันแลกแต้มทั้งหมดเป็นเครดิตใช่ไหม?',
      detail: 'ระดับบัญชีอาจลดลงตามยอดที่แลก',
      confirmText: 'แลกแต้ม',
      cancelText: 'ยกเลิก',
      variant: 'warning',
    });
    ...
    setMsg(null);
    try {
      const res = await fetch('/api/account/points/redeem', { method: 'POST' });
      const data = await res.json();
      if (!res.ok || !data.ok) {
        notifyMsg(setMsg, { type: 'error', text: data.error || data.message || 'แลกแต้มไม่สำเร็จ' });
        return;
      }

      notifyMsg(setMsg, { type: 'success', text: data.message || 'แลกแต้มเป็นเครดิตสำเร็จ' });
      if (data.user) setUser(data.user);
      else await loadProfile();
      router.refresh();
    } catch (e) {
      notifyMsg(setMsg, { type: 'error', text: 'แลกแต้มไม่สำเร็จ กรุณาลองใหม่' });
    } finally {
      setBusy(false);
    }
  }

  async function withdrawAffiliateToBalance() {
    if (busy) return;
    const amount = Number(affiliateData?.withdrawableTHB || 0);
    if (amount <= 0) {
      notifyMsg(setMsg, { type: 'warn', text: 'ยังไม่มียอดรายได้ที่ถอนได้' });
      return;
    }

    const ok = await confirmAction({
      title: 'ถอนรายได้แนะนำเพื่อน',
      message: `ยืนยันถอนรายได้แนะนำเพื่อนเข้าเครดิตใช่ไหม?`,
      detail: `ยอดที่จะถอน: ฿${amount.toLocaleString('th-TH', { minimumFractionDigits: 2 })}`,
      confirmText: 'ถอนรายได้',
      cancelText: 'ยกเลิก',
      variant: 'success',
    });
    if (!ok) return;
    ...
    try {
      const res = await fetch('/api/account/affiliate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'withdraw-balance' }),
      });
      const data = await res.json();
      if (!res.ok || !data.ok) {
        notifyMsg(setMsg, { type: 'error', text: data.error || data.message || 'ถอนรายได้ไม่สำเร็จ' });
        return;
      }

      notifyMsg(setMsg, { type: 'success', text: data.message || 'ถอนรายได้เข้าเครดิตสำเร็จ' });
      setAffiliateData(data.summary || null);
      setUser((u) => ({ ...u, balance: data.balance ?? u?.balance }));
      await loadAffiliate();
      router.refresh();
    } catch (e) {
      notifyMsg(setMsg, { type: 'error', text: 'ถอนรายได้ไม่สำเร็จ กรุณาลองใหม่' });
    } finally {
      setBusy(false);
    }
  }
                    ...
                    <div>
                      ...
                      <div className="card-actions" style={{ textAlign: 'right', padding: '12px 0 0' }}>
                        <button
                          ...
                          onClick={redeemPointsToBalance}
                          title={points < 100 ? `ต้องมีอย่างน้อย 100 แต้ม (ขาดอีก ${Math.max(0, 100 - points)} แต้ม)` : ''}>
                          {busy ? 'กำลังดำเนินการ...' : 'แลกแต้มเป็นเครดิต (ขั้นต่ำ 100 แต้ม)'}
                        </button>
                      </div>
                    </div>
                  </div>
                ...
                <div className="form-grid">
                  <div className="muted full-col">
                    <ul style={{ margin: '0 0 0 18px', padding: 0 }}>
                      ...
                      <li>ตัวอย่างรูปแบบลิงก์: <code>https://rtautobot.com/aff?aff=รหัสแนะนำเพื่อนของคุณ</code></li>
                    </ul>
                  </div>
                </div>
              </div>
                  ...
                  <div className="summary-kpi">
                    ...
                    <div className="kpi-actions">
                      ...
                      <button
                        ...
                        disabled={Number(affiliateData?.withdrawableTHB || 0) <= 0 || busy}
                        onClick={withdrawAffiliateToBalance}>
                        {busy ? 'กำลังดำเนินการ...' : 'ถอนรายได้'}
                      </button>
                    </div>
                  </div>
                </div>
                  ...
                  {affUrl ? (
                    <div className="full-col">
                      ...
                      <div className="card-actions" style={{ textAlign: 'right', padding: '0', marginTop: '12px' }}>
                        <button className="btn" onClick={() => copyTextWithNotify(affUrl, 'คัดลอกลิงก์แนะนำเพื่อนแล้ว')}>คัดลอกลิงก์</button>
                      </div>
                    </div>
                  ) : (
                    <div className="full-col">
