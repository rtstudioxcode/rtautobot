...
export default function ResetPasswordPage() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (password !== confirm) { notifyMsg(setMsg, { ok: false, text: 'รหัสผ่านไม่ตรงกัน' }); return; }
    if (password.length < 8) { notifyMsg(setMsg, { ok: false, text: 'รหัสผ่านต้องมีอย่างน้อย 8 ตัวอักษร' }); return; }
    ...
    try {
      ...
      if (d.ok) {
        notifyMsg(setMsg, { ok: true, text: 'เปลี่ยนรหัสผ่านสำเร็จ กำลังเข้าสู่ระบบ...' });
        setTimeout(() => router.push('/dashboard'), 1500);
      } else {
        notifyMsg(setMsg, { ok: false, text: d.message || 'ลิงก์ไม่ถูกต้องหรือหมดอายุแล้ว' });
      }
    } catch {
      notifyMsg(setMsg, { ok: false, text: 'เกิดข้อผิดพลาด กรุณาลองใหม่' });
    } finally {
      setLoading(false);
    }
  }
