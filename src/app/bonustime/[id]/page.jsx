...
export default function BonustimeDetailPage() {
  ...
  async function handleUpgradeLotto() {
    if (!confirm('ยืนยันการอัปเกรดเป็นแพ็กเกจรวมหวย? ราคา ฿1,000')) return;
    setBusy(true); setMsg(null);
  ...
  async function handleRestart() {
    ...
    if (!confirm('ยืนยัน Restart Service?')) return;
    setBusy(true); setMsg(null);
  ...
  return (
    <>
      ...
      <div className="btd-page">
        ...
        {/* Flash Message */}
        {msg && <div className={`btd-flash ${msg.type}`}>{msg.text}</div>}
