...
export async function POST(request) {
  try {
    ...
    const topup = await Topup.findOne({ production: PRODUCTION_KEY, accountCode: 'scb', isActive: true }).lean();
    ...
    const [, day, month, hour, minute, amountStr, bank, senderDigits, receiverDigits] = match;
    ...
    if (!amt) return NextResponse.json({ success: false }, { status: 400 });

    const amtCents = Math.round(amt * 100);
    const seconds = timestamp ? new Date(timestamp * 1000).getSeconds() : new Date().getSeconds();
    const parsedDate = makeBangkokDate(new Date().getFullYear(), Number(month), Number(day), Number(hour), Number(minute), seconds);
    ...
    console.error('SCB WEBHOOK ERROR', e);
