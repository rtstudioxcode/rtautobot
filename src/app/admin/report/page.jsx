...
import ReportClient from './ReportClient.jsx';
...
export default async function AdminReportPage({ searchParams }) {
  ...
  if (mode === 'month') {
    ...
    const baseMatch = { production: 'rtautobot', status: 'completed', ...dateFilter };
    ...
    const [txDocs, aggNoAdmin, aggMethod] = await Promise.all([
      ...
      Transaction.aggregate([
        { $match: { ...baseMatch, method: { $nin: ['admin', 'manual'] } } },
    ...
    const baseMatch = { production: 'rtautobot', status: 'completed', createdAt: { $gte: startDate, $lt: endDate } };
