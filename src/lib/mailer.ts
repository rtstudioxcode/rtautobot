...
export function getTransporter() {
  ...
  transporter.verify().catch(err =>
    glog.error('[mailer] verify failed:', err?.message || err)
  );
...
export async function sendEmail({ to, subject, html, text, headers, attachments }: any = {}) {
  // ⬇️ ถ้ายังไม่มี SMTP config ให้ลองดึงจาก DB แบบ lazy
  if (!config.mail?.host || !config.mail?.port) {
    try {
      const { refreshConfigFromDB } = await import('../config');
  ...
  while (attempt < MAX_ATTEMPTS) {
    try {
      ...
      if (config.mail?.debug) {
        glog.warn(`[mailer] send fail (attempt ${attempt}) -> retry in ${wait}ms:`,
