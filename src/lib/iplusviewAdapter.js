...
function normCancelCreateResp(data) {
  ...
  return {
    cancelId:
      ...
      data?.cancel_id ??
      null,
    status: String(first?.status ?? data?.status ?? "").toLowerCase(),
    raw: data,
  };
}

async function getProviderRuntime() {
  let baseURL = trimBase(
    config?.provider?.baseUrl ||
    config?.provider?.baseURL ||
    ""
  );

  let apiKey = String(config?.provider?.apiKey || "").trim();
  ...
  if ((!baseURL || !apiKey) && !_triedRefreshOnce) {
    ...
    try {
      ...
      apiKey = String(config?.provider?.apiKey || "").trim();
    } catch {
      // noop
    }
  }
...
async function getClient() {
  ...
  if (!baseURL || !apiKey) {
    throw new Error(
      "Provider credentials are not configured in secure_config (ipv.apiBase / ipv.apiKey missing)."
    );
  }
...
function normalizeStatus(data) {
  const raw = String(data?.status || data?.order_status || "")
    .toLowerCase()
    .trim();
...
export async function getBalance() {
  ...
  try {
    const r1 = await client.get("/balance");
    return r1.data;
  } catch {
    const r2 = await client.get("/credit");
    return r2.data;
  }
}
...
export async function createOrder(payload) {
  ...
  if (!quantityNum) {
    throw new Error("createOrder: quantity is required");
  }
  ...
  if (!providerOrderId) {
    throw new Error("createOrder: provider did not return order id");
  }
...
export async function getRefillStatus(refillIdOrOrderId) {
  if (!refillIdOrOrderId) {
    throw new Error("getRefillStatus: id is required");
  }
  ...
  const { data } = await client.get(
    `/orders/${refillIdOrOrderId}/refill-status`
  );
...
export async function createCancel(providerOrderId) {
  if (!providerOrderId) {
    throw new Error("createCancel: providerOrderId is required");
  }

  const client = await getClient();

  const { data } = await client.post(
    `/cancels/${encodeURIComponent(String(providerOrderId))}`,
    {}
  );
...
export async function getCancelById(cancelId) {
  if (!cancelId) {
    throw new Error("getCancelById: cancelId is required");
  }

  const client = await getClient();

  const { data } = await client.get(
    `/cancels/${encodeURIComponent(String(cancelId))}`
  );
  ...
  return {
    ...
    type: String(data?.type || data?.refund_type || ""),
    raw: data,
  };
}
...
export async function findCancelsByIds(ids = []) {
  ...
  try {
    const { data } = await client.get("/cancels", {
      params: { ids: list },
    });
