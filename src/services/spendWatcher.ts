      ...
      if (all.length) {
        ...
        for (const doc of all) {
          ...
          (async () => {
            try {
              if (otpSet.has(String(doc._id))) {
                const { reconcileOtp24OrderSpend } = await import('./spend');
                await (reconcileOtp24OrderSpend as any)(doc._id);
              } else {
                const { reconcileOrderSpend } = await import('./spend');
                await (reconcileOrderSpend as any)(doc._id);
              }
            } catch (e: any) {
          ...
          scheduleUser(uid, async () => {
            try {
              const { recalcUserTotals } = await import('./spend');
              await (recalcUserTotals as any)(uid, { force: true, reason: 'polling' });
            } catch (e: any) {
      ...
      try {
        ...
        (async () => {
          try {
            if (kind === 'orders') {
              const { reconcileOrderSpend } = await import('./spend');
              await (reconcileOrderSpend as any)(orderId);
            } else {
              const { reconcileOtp24OrderSpend } = await import('./spend');
              await (reconcileOtp24OrderSpend as any)(orderId);
            }
          } catch (e: any) {
        ...
        scheduleUser(uid, async () => {
          try {
            const { recalcUserTotals } = await import('./spend');
            await (recalcUserTotals as any)(uid, { force: true, reason: `change_stream_${kind}` });
          } catch (e: any) {
