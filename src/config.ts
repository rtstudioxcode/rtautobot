...
export const config: any = structuredClone(envConfig);
...
export async function refreshConfigFromDB({ markHash = true }: any = {}) {
  try {
    const doc = await (SecureConfig as any).findOne().lean();
...
export function startSecureConfigAutoReload({ onReload }: any = {}) {
...
export async function getSecureConfigDoc() {
  try {
    return await (SecureConfig as any).findOne().lean();
  } catch {
    return null;
  }
}
